...
const NumberTicker = ({style, textSize = 35, textProportion = 100, textStyle, number, duration}) => {
...
class TextTicker extends Component {
    ...
    render() {
        const {animatedValue} = this.state;
        const styles = generateStyles(this.props.textSize, this.props.textProportion);
...
const generateStyles = (textSize, textProportion) => StyleSheet.create({
    container: {
        width: textSize * textProportion,
        height: textSize,
        overflow: 'hidden',
        alignItems: 'center',
